...
DECL *make_procfunc_decl (IDENT *id, PROCFUNCDEF *p)
{
...
PROCFUNCDEF *make_profunc (DECLLIST *dl, TYPE *t, BLOCK *b)
{
	PROCFUNCDEF *p = anew(PROCFUNCDEF);
	
	p->proc_formals = dl;
	//if( t != 0)
		p->proc_result_type = t;
	/*else
		d->d.proc->proc_result_type = NULL;*/
	p->proc_block = b;
...
TYPE *make_elt_type (TYPE *idx, TYPE *elt)
{
  TYPE *t = make_type (TypeArray_);
  t->t.arr = make_typearray(idx, elt);
  t->size += 1;
...
FIELDLIST *make_fieldlist(FIELD *f, FIELDLIST *flist)
{
	FIELDLIST *fflist = anew(FIELDLIST);
...
STMTIF *make_stmtif (EXPR *test, STMT *th, STMT *el)
{
...
STMTREPEAT *make_stmtrepeat (EXPR *test, STMTLIST *slist)
{
...
STMTFOR *make_stmtfor (BINDING *b, EXPR *e1, EXPR *e2, STMT *s)
{
  STMTFOR *sfor = anew (STMTFOR);
  sfor->for_name = b;
  sfor->for_init = e1;
  sfor->for_to = e2;
  sfor->for_stmt = s;
  sfor->for_upward = TRUE;
  return sfor;
}
...
STMT *make_ifstmt (EXPR *test, STMT *th, STMT *el)
{
  STMT *s = make_stmt (StmtIf_);
  s->s.ifx = make_stmtif (test, th, el);
  return s;	
}

//featrue8

STMT *make_casestmt(EXPR *test, CASELIST *clist)
{
  STMT *s = make_stmt (StmtCase_);
  s->s.cas = make_stmtcase (test, clist);
  return s;	
...
STMT *make_repeatstmt (EXPR *test, STMTLIST *slist)
{
	STMT *s = make_stmt (StmtRepeat_);
	s->s.rpt = make_stmtrepeat(test, slist);
	return s;
}

//featrue10

STMT *make_forstmt (IDENT *id, EXPR *e1, EXPR *e2, STMT *s)
{
	STMT *sfor = make_stmt (StmtFor_);
	sfor->s.forx = make_stmtfor(make_binding (id), e1, e2, s);
	return sfor;
}

//featrue6

STMT *make_compstmt (STMTLIST *slist)
{
...
STMTLIST *make_stmtlist(STMT *s, STMTLIST *slist)
{
	STMTLIST *sslist = anew(STMTLIST);
...
CASELIST *make_caselist (CASEARM *ca, CASELIST *clist)
{
	CASELIST *cclist = anew(CASELIST);
